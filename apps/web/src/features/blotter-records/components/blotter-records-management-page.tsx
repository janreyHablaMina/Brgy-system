...
export function BlotterRecordsManagementPage() {
  ...
  function exportData(format: "csv" | "excel") {
    ...
    if (format === "csv") {
      downloadCsv(`blotter-records-${stamp}.csv`, rows);
      return;
    }
    downloadExcelCompatible(`blotter-records-${stamp}.xls`, rows);
  }
    ...
    <section className="space-y-6">
      <header className="px-1">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-[var(--text)]">Blotter Records</h1>
            <p className="mt-1 text-xs text-[var(--muted)]">Track barangay blotter entries, mediation notes, and incident status updates.</p>
          </div>
          <DropdownMenu
