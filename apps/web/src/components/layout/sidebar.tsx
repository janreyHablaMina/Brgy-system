...
import { APP_NAVIGATION } from "@/core/navigation/route-registry";
...
export function Sidebar({ collapsed, mobileOpen, onToggleCollapse, onCloseMobile }: SidebarProps) {
  ...
  const activeMenus = useMemo(
    () =>
      APP_NAVIGATION.flatMap((group) => group.items)
  ...
  return (
    <>
      ...
      >
        <div className={cn("relative flex flex-col items-center justify-center pt-10 pb-6 transition-all duration-300", collapsed ? "px-2" : "px-6")}>
          <button
          ...
          <div className="relative group">
            ...
            <Image
              ...
              width={80}
              height={80}
              className={cn("relative z-10 transition-all duration-500", collapsed ? "h-12 w-12" : "h-20 w-20")}
          ...
          {!collapsed && (
            <div className="mt-4 text-center animate-in fade-in slide-in-from-top-2 duration-700">
              ...
              <div className="h-0.5 w-12 mx-auto mt-2 rounded-full" style={{ backgroundColor: "var(--primary)", boxShadow: "0 0 8px var(--primary)" }} />
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col flex-1 overflow-y-auto px-6 py-4 custom-scrollbar">
          {APP_NAVIGATION.map((group) => (
            <div key={group.title} className="mb-6">
              {!collapsed && <p className="mb-4 text-sm font-semibold text-[#5A6F8A]">{group.title}</p>}
                    ...
                    <div key={item.label}>
                      {hasChildren ? (
                        ...
                        >
                          ...
                          {!collapsed && (isOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
                        </button>
                      ) : (
                        <Link
                          href={item.href || "#"}
